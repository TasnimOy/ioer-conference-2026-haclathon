...
*   **[Topic: Settlement Monitoring]** - *Oliver Harig*
    IBTool is an open-source QGIS plugin that automatically derives settlement boundaries from building footprints and street networks (see {cite:alp}`ijgi10050353`). It replaces manually digitised or administratively defined delineations with a consistent, data-driven delineation of the inner zone (§ 34 BauGB). Practical applications include settlement-area monitoring, assessing infill-development potential, and documenting land consumption — scalable from the municipal to the nationwide level. A recent expert evaluation ({cite:alp}`repec:sae:envirb:v:52:y:2025:i:7:p:1735-1755`) found IBTool's delineations more precise and consistent than comparable products such as ATKIS®-Ortslage and GHSL, with notably less over-detection at settlement edges. To support practical use, I will contribute tutorials covering the application of the tool as well as guidance on data preparation.
...
*   **[Topic: From Footprints to Building Stock Insights: A Spatial Decision-Support Pipeline for Sustainable Transformation]** - *Markus Münzinger; Martin Behnisch (IOER)*
